...
class HeaderTreeNode extends LayerText {
    constructor(protected props: IHeaderNodeProps) {
        super(props);
        this.style.backgroundColor = props.style?.backgroundColor ?? this.table.style.headerBackColor;
        this.style.fontWeight = props.style?.fontWeight ?? 'bold';

        this.parentCell = props.parent;

        // добавляем себя в дочерние элементы родительского элемента
        if (this.parentCell) {
